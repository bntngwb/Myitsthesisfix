...
export default function App() {
  ...
  const [userRole, setUserRole] = useState<"Mahasiswa" | "Dosen" | "Admin">("Mahasiswa");
  ...
  const [selectedHearingEvent, setSelectedHearingEvent] = useState<HearingEvent | null>(null);
  ...
  const announcements: Announcement[] = [
    {
      ...
      content: "Jadwal sidang proposal gelombang 2 mengalami perubahan dari tanggal 25 November menjadi 28 November 2024. Mahasiswa yang telah mendaftar sidang diharapkan untuk menyesuaikan jadwal. Konfirmasi ulang akan dikirimkan melalui email resmi masing-masing. Untuk informasi lebih lanjut, silakan hubungi sekretariat program studi.",
      imageUrl: "https://images.unsplash.com/photo-1649645669515-85299ad3688b?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxhbGVydCUyMHdhcm5pbmclMjBub3RpZmljYXRpb258ZW58MXx8fHwxNzYzNzEzMjk4fDA&ixlib=rb-4.1.0&q=80&w=1080",
      isNew: true
    },
    {
      id: 2,
      ...
      content: "Batas akhir pengumpulan proposal tugas akhir untuk semester gasal 2024/2025 adalah tanggal 30 November 2024 pukul 23:59 WIB. Proposal yang terlambat dikumpulkan tidak akan diproses dan harus menunggu periode berikutnya. Pastikan semua dokumen sudah lengkap dan telah mendapatkan persetujuan dari dosen pembimbing sebelum dikumpulkan.",
      imageUrl: "https://images.unsplash.com/photo-1624969862293-b749659ccc4e?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxkZWFkbGluZSUyMGNhbGVuZGFyJTIwcmVtaW5kZXJ8ZW58MXx8fHwxNzYzNzEzMjk0fDA&ixlib=rb-4.1.0&q=80&w=1080",
      isNew: true
    },
    {
      id: 3,
      ...
      content: "Departemen Informatika mengadakan Seminar Nasional Teknologi Informasi dengan tema 'AI and Machine Learning in Modern Era' pada tanggal 5 Desember 2024. Acara ini terbuka untuk mahasiswa S1 dan S2. Pembicara: Prof. Dr. Ahmad Saikhu, M.Kom dan Dr. Retno Wardani, S.Kom, M.T. Pendaftaran dibuka hingga 1 Desember 2024. Fasilitas: sertifikat, coffee break, dan materi seminar.",
      imageUrl: "https://images.unsplash.com/photo-1759456629070-8e222ab878ba?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxhY2FkZW1pYyUyMGV2ZW50JTIwc2VtaW5hcnxlbnwxfHx8fDE3NjM3MTMyOTN8MA&ixlib=rb-4.1.0&q=80&w=1080",
      isNew: false
    },
    {
      id: 4,
    ...
    },
    {
      ...
      content: "Sistem myITS Thesis akan menjalani maintenance rutin pada hari Minggu, 17 November 2024 pukul 00:00 - 06:00 WIB. Selama periode ini, sistem tidak dapat diakses. Harap melakukan penyimpanan data dan submit proposal sebelum waktu maintenance dimulai. Mohon maaf atas ketidaknyamanannya. Untuk pertanyaan, hubungi tim IT support.",
      imageUrl: "https://images.unsplash.com/photo-1758780690553-8bc703fabca6?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxzeXN0ZW0lMjBtYWludGVuYW5jZSUyMHVwZGF0ZXxlbnwxfHx8fDE3NjM3MTMyOTR8MA&ixlib=rb-4.1.0&q=80&w=1080",
      isNew: false
    }
  ];
  ...
  const availableHearings: Hearing[] = [
    {
      ...
      description: "Sidang proposal untuk mahasiswa yang telah menyelesaikan pengajuan proposal tugas akhir. Mahasiswa akan mempresentasikan proposal di hadapan dosen penguji.",
    ...
    },
    {
      ...
      batch: 1,
      period: "Semester Gasal 2025/2026",
      description: "Sidang akhir untuk mahasiswa yang telah menyelesaikan penelitian dan penulisan tugas akhir. Mahasiswa akan mempresentasikan hasil penelitian secara lengkap.",
    ...
    },
    {
      ...
      description: "Sidang proposal gelombang kedua untuk mahasiswa yang mengajukan proposal di semester genap.",
  ...
  const handleSaveProposal = (proposalData: any, isDraft: boolean) => {
    ...
    if (!isDraft) {
      ...
      approvalDeadline = deadline.toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' });
    }
    ...
    const newProposal: Proposal = {
      ...
      supervisor1Approval: !isDraft ? (editingProposal?.supervisor1Approval || "pending") : editingProposal?.supervisor1Approval,
      supervisor2Approval: !isDraft ? (editingProposal?.supervisor2Approval || "pending") : editingProposal?.supervisor2Approval,
      adminApproval: !isDraft ? (editingProposal?.adminApproval || "pending") : editingProposal?.adminApproval,
      approvalDeadline: !isDraft ? (editingProposal?.approvalDeadline || approvalDeadline) : editingProposal?.approvalDeadline,
  ...
  const handleTakeHearing = (hearingId: number, proposalId: number) => {
    ...
    const newTakenHearing: TakenHearing = {
      ...
      approvalDeadline: approvalDeadline.toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }),
    };
  ...
  const handleCompleteProposalDefense = (proposalId: number) => {
    // Move proposal from "proposal" stage to "final" stage
    setProposals(proposals.map(p =>
      p.id === proposalId
        ? { ...p, stage: "final", status: "Tugas Akhir - Dalam Pengerjaan" }
        : p
    ));
    
    // Update the hearing status to completed
    setTakenHearings(takenHearings.map(h =>
    ...
    setTakenHearings(takenHearings.map(h =>
      h.id === hearingId
        ? { 
            ...
            revisionApprovalDeadline: revisionApprovalDeadline.toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })
          }
        : h
    ));
    // Also update selectedTakenHearing if it's the one being updated
  ...
  // Handle hash navigation for sidang detail URLs
  useEffect(() => {
  ...
  const handleNavigate = (page: string, announcementId?: number) => {
    if (page !== "Pengumuman" && page !== "Buat Pengumuman Baru" && page !== "Edit Pengumuman" && page !== "Kelola Pengumuman") {
    ...
    <div className="min-h-screen bg-[#f5f5f5] flex flex-col pt-[57px]">
      ...
      <div className="flex flex-1 relative">
        ...
        <div className="flex-1 ml-64">
          ...
          {currentPage === "Beranda" && userRole === "Dosen" && (
            <BerandaDosen
              ...
              onAnnouncementClick={(id) => {
                ...
                // The announcement will be highlighted/opened in the Pengumuman component
              }}
            />
          )}
          ...
          {currentPage === "Sidang" && userRole === "Dosen" && !currentPage.includes("/") && (
            <SidangDosen />
          )}
          ...
          {currentPage === "Pengumuman" && userRole !== "Admin" && <Pengumuman announcements={announcements} />}
          {currentPage === "Pengumuman" && userRole === "Admin" && !showCreatePengumuman && (
            <div className="p-8">
          ...
          {currentPage === "Jadwal Sidang" && userRole === "Admin" && !selectedHearingEvent && (
            <JadwalSidangAdminCalendar />
          )}
          ...
          {currentPage === "Administrasi" && userRole === "Admin" && <Administrasi />}
          {currentPage === "Alokasi Pembimbing" && userRole === "Admin" && <AlokasiPembimbing />}
          {currentPage === "Pembimbingan Dosen" && userRole === "Admin" && <PembimbinganDosenAdmin />}
          {currentPage === "Tugas Akhir Mahasiswa" && userRole === "Admin" && <TugasAkhirMahasiswa />}
          {currentPage === "Rekapitulasi Nilai" && userRole === "Admin" && <RekapitulasiNilai />}
          {currentPage === "Konfigurasi Prodi" && userRole === "Admin" && <KonfigurasiProdi onNavigate={handleNavigate} />}
          {currentPage === "Konfigurasi Nilai" && userRole === "Admin" && <KonfigurasiNilai onNavigate={handleNavigate} />}
          {currentPage === "Konfigurasi myITS Thesis" && userRole === "Admin" && <KonfigurasiMyITS onNavigate={handleNavigate} />}
          {currentPage === "Konfigurasi Prasyarat" && userRole === "Admin" && <KonfigurasiPrasyarat onNavigate={handleNavigate} />}
          {currentPage === "Konfigurasi Sidang" && userRole === "Admin" && <KonfigurasiSidang onNavigate={handleNavigate} />}
          {currentPage === "Kelola Pengumuman" && userRole === "Admin" && <PengumumanAdminList onNavigate={handleNavigate} />}
          {currentPage === "Buat Pengumuman Baru" && userRole === "Admin" && <PengumumanAdminEdit onNavigate={handleNavigate} />}
          {currentPage === "Edit Pengumuman" && userRole === "Admin" && <PengumumanAdminEdit onNavigate={handleNavigate} announcementId={editingAnnouncementId} />}
          {currentPage === "Data Prodi" && userRole === "Admin" && <DataProdi onNavigate={handleNavigate} />}
        </div>
      </div>
      <Toaster />
    </div>
